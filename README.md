...
- **パフォーマンス最適化**:
  - `useRef`: Canvas 描画やタイマー管理など、再レンダリングを避けるべき処理に活用し、State の自動管理の競合しないように使用を意識。
...
- **初回ログイン時設定から名前変更出来る → ゲームスタート押すともう一回聞かれる**
  - 初回ログイン時は設定を開けないように変更
- **設定から名前変更した時に分かりづらい**
  - ✓ 保存しました！のテキストが出るように変更

---

**今後の改善予定**

- 難易度選択の全国ランキングを一画面で見れるようにする。
- 設定画面で名前保存した時に、保存しましたの表示を入れる。
- LP 制作
