...
- 今回の構成では CIDR で制御することが難しい。Lambda VPC は 関数作成時やVPC設定更新時に ENI を作成し、IP アドレスは AWS 側で発行する為、自分たちで管理が出来ない。個別で IP アドレスを絞れないため、同一サブネット単位で許可する方向に倒れる。（例：10.0.1.0/24）  
