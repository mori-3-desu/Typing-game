...
以下のフックに分割し、見通しの良い構造に整理していきます。

- `useRankingController`（ランキング取得・モード切替・ローディング）
- `useTitleFlow`（名前入力/確認フロー）
- `useBootstrap`（初期データ取得/ローディング遷移）

**完了条件**: `App.tsx` の state/handler を 30% 以上削減し、既存機能とテストが全て通ること。
