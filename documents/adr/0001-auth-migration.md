...
Accepted (2026-05-26) — Phase 1 実装中（認証基盤 + テスト完了 2026-05-29、残項目は下記 Migration Strategy > Phase 1 参照）
...
| 新規: `JwtIssuer` Service      | JWT 署名発行ロジック（純粋関数として単体テスト可能に） 不正エンコード、発行アルゴリズム対策 ⭐                                                                                                                                                                                                                                                          |
...
> **Phase 1 実装ステータス（2026-05-29 時点）**
>
> ✅ **実装完了**: 鍵管理（`JwtKeySet` / `JwtKeyConfig` / `generate-jwt-keys.sh`）、`JwtAuthenticationFilter` 改修（Cookie 読み取り + ES256 検証 + HS256 撤去）、`SecurityConfig`（認可ルール・CSRF Double Submit・CORS・`ApiAuthenticationEntryPoint`(401) / `ApiAccessDeniedHandler`(403)）。SecurityConfig は Educational Anti-Pattern サイクル（わざと脆弱に書く→攻撃成立→1つずつ閉じる）で検証。
>
> ✅ **テスト完了（2026-05-29）**: `JwtAuthenticationFilter` 単体テスト 6 本 ＋ `SecurityConfig` MockMvc 統合テスト 6 本（`spring-security-test` を test scope で追加）。`mvnw test` 全体緑。
>

> Cookieをsliding方式に変更、認証済みで再発行する方針、長期プレイヤーのセッション維持によるデータ損失を防ぐ。引継ぎコードで損失の際の保険を実装済みである。
> 未ログインが長いゲストユーザーを自動でデータベースからデータを消してもいいかもしれない。

> 検証ログ・学び: `learnings/security/spring-security-r1-r2-config-and-filter.md`、`learnings/security/spring-security-r5-error-handling.md`、`learnings/sessions/2026-05-29.md`（テストフェーズ）。
